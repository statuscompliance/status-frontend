...
export default function Chat() {
  ...
  const errorMessage =
    "El texto es demasiado corto, por favor, inténtalo de nuevo.";
    ...
    setTimeout(async function retry() {
      try {
        ...
        if (response.message === "Run not completed yet") {
          if (retryInterval < maxRetryInterval) {
            ...
            setThreadMessages([
              {
                role: "assistant",
                content: `Generando respuesta, espere ${
                  retryInterval / 1000
                } segundos...`,
                isError: false,
              },
                ...
                setThreadMessages([
                  {
                    role: "assistant",
                    content:
                      "El mashup que has solicitado ya existe o existe uno similar",
                    isError: false,
                  },
                ]);
              }
        ...
        setThreadMessages([
          {
            role: "assistant",
            content: "Error al obtener mensajes del hilo.",
            isError: true,
          },
        ]);
      }
        ...
        <div className="editorContainer">
          {animated && isNodeRedDeployed ? (
            <div className={`chatEditor ${animated ? "visible" : ""}`}>
              <iframe
                src={`${process.env.REACT_APP_NODE_RED_URL || 'http://localhost:1880'}${endpoint}`}
                title="Editor"
          ...
          <div
            className={`loader ${animated && !hideChat ? "" : "hide"}`}
          ></div>
          <div
            className={`chat ${
              animated && !hideChat ? "blur" : hideChat ? "hide" : ""
            }`}
          >
            <div className="historyContainer">
              {existThreads(threads) ? (
                ...
                <div>
                  <p className="historyEmpty">No hay conversaciones en curso</p>
                </div>
              )}
            </div>
